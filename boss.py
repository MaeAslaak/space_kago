...
def lancer_boss():
    ...
    nonlocal_vars = {'player_hp': player_hp, 'boss_hp': boss_hp, 'selected_index': selected_index}
        ...
        for event in pygame.event.get():
            ...
            if not game_over and event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_LEFT, pygame.K_RIGHT, pygame.K_UP, pygame.K_DOWN]:
                    nonlocal_vars['selected_index'] = handle_input(event.key, nonlocal_vars['selected_index'])
                elif event.key == pygame.K_RETURN:
                    choice = get_letter(nonlocal_vars['selected_index'])
                    if choice == question["answer"]:
                        damage = random.randint(10, 20)
                        nonlocal_vars['boss_hp'] -= damage
                    else:
                        damage = random.randint(5, 15)
                        nonlocal_vars['player_hp'] -= damage

                    if nonlocal_vars['player_hp'] <= 0:
                        game_over = True
                        victory = False
                    elif nonlocal_vars['boss_hp'] <= 0:
                        game_over = True
                        victory = True
